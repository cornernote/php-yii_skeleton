<?php
/**
 * SaveRelationsBehavior class file.
 *
...
/*
 * The SaveRelationBehavior enables ActiveRecord model to save HasMany and ManyMany 
 ...
 * 		return array('SaveRelationsBehavior' => array('class' => 'application.models.behaviors.SaveRelationsBehavior'));
 * }
 * In your controller, to save the relations data, simply call
            ...
            foreach ($this->relations as $relation => $params) {
                if (isset($params['save']) && $params['save'] == true) {
                    ...
                    if ($activeRelation instanceOf CManyManyRelation) {
                        ...
                        preg_match('/^\s*(.*?)\((.*)\)\s*$/', $relationForeignKey, $matches);
                        $joinTable = $schema->getTable($matches[1]);
                        $joinClassName = str_replace(' ', '', ucwords(str_replace('_', ' ', $matches[1])));
                        $fks = preg_split('/[\s,]+/', $matches[2], -1, PREG_SPLIT_NO_EMPTY);
                        if (!class_exists($joinClassName)) {
                            ...
                            // use a join model
                            foreach ($model->{$relation} as $relatedRecord) {
                                $keysToKeep[$fks[1]][] = $relatedRecord->primaryKey;
                                $attributes = array(
                                    $fks[0] => $model->primaryKey,
                                    $fks[1] => $relatedRecord->primaryKey,
                                );
                                $joinRecord = $joinClassName::model()->findByAttributes($attributes);
                                if (!$joinRecord) {
                                    $joinRecord = new $joinClassName;
                            ...
                            $joinRecord = new $joinClassName;
                            $criteria = new CDbCriteria;
                            $criteria->addColumnCondition(array($fks[0] => $model->primaryKey));
                            foreach ($keysToKeep as $fk => $values)
                                $criteria->addInCondition($fk, $values, 'AND NOT');
                            $joinRecord->deleteAll($criteria);
                        }
                    }
                    else {
