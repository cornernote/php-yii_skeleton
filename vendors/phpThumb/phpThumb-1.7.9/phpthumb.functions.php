...
class phpthumb_functions {
    ...
    static function versionProblem($command)
    {
        $return = false;
        $problemCommands['convert --version']='convert.version';
        $file_path=dirname(__FILE__) . '/ignore_image_magic.txt';
        if (file_exists($file_path)){
            if (isset($problemCommands[$command])){
                return "don't use phpThumb";
            }
        }
        return $return;
    }
	...
	static function SafeExec($command) {
        ...
        $versionProblem= self::versionProblem($command);
        if ($versionProblem){
